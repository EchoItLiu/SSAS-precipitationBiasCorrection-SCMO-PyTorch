...
'''
python pre_training/MSM_pretraining/T_MSM_Pre.py -c ./config/SHO.yaml -r 0.3 -d 8 9 
'''
...
if len(args.device_ids) > 1:
    tem_MSM = nn.DataParallel(tem_MSM, device_ids=args.device_ids).cuda()

# else:
tem_MSM = tem_MSM.cuda()
    
# print ('tem_MSM_params:', tem_MSM.state_dict())
# print ('tem_MSM_params_type:', type(tem_MSM))
print(args)

## 增加模块优化器(5个/***或考虑1-2个做联合(jointly)优化，后期考虑参数共享部分)和实验超参设置
tem_optimizer = optim.Adam(tem_MSM.parameters(), lr=args.learning_rate, weight_decay=params_SAS['weight_decay'])
##
scheduler_tem = StepLR(tem_optimizer, step_size=params_SAS['lr_decay_epoch'], gamma=params_SAS['lr_decay'])
## Transform
utilPath = osp.join(
    args.basePath,
    args.dataset_name,
...
print ('Pre-Training temperature_MSM...')
for epoch in range(args.preEpoch):
    ...
    for batch_idx, (data, seqPrep, _, seqTem, seqPress, seqWind, seqDew) in enumerate(OrdDataLoader):
        ...
        data_tem, temTarget = updateMeBatch4Label(data, seqTem, args.dropRate, 'tem')
        ...
        for ci, curScale in tqdm.tqdm(
            ...
            ## Crop current scale
            curScale_tem = cropOneScale4EC(data_tem, curScale, EC_center)
            ...
            #
            tem_outputs = tem_MSM(curScale_tem)
            # BP
            BP_ME(tem_outputs, temTarget, tem_optimizer, 'tem')
    ...
    if epoch%params_SAS['training_inv']==0:
        ...
        for batch_idx, (data, seqPrep, _, seqTem, seqPress, seqWind, seqDew) in enumerate(modalGD_FDataLoader):
            ...
            for ci, curScale in tqdm.tqdm(
                ...
                ## Inference
                with torch.no_grad():
                    t_outputs = tem_MSM(curScale_tem)
                    #
                    t_loss = F.mse_loss(t_outputs, temTarget).item()
                    #
        ...
        if epoch>=3 and t_bestLoss > t_loss_avg:
            ...
            tem_weights = tem_MSM.state_dict()
            print('saving-epoch-{}-Test-temLoss-{}'.format(epoch + 1, t_bestLoss))
            # spatial module for temperature and override
#             torch.save(tem_weights, osp.join(SMS_save_path, 'tem', 'pretrained-MSM-tem.pth'))
            torch.save(tem_weights, osp.join(SMS_save_path, 'tem', 'pretrained-MSM-tem_preserve.pth'))    
