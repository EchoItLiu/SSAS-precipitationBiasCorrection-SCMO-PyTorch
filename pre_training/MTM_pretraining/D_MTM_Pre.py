...
'''
python pre_training/MTM_pretraining/D_MTM_Pre.py -c ./config/SHO.yaml -r 0.3 -d 6 7 8
'''
...
parser.add_argument("--preEpoch", help='pretraining epoch', type=int, default=100000000)
...
dewRange = np.arange(params_STAS['set_min_CdewValue'],params_STAS['set_max_CdewValue']+params_STAS['set_Cdew_space'], params_STAS['set_Cdew_space'])
...
dew_optimizer = torch.optim.Adam([
...
##
scheduler_dew = StepLR(dew_optimizer, step_size=params_STAS['lr_decay_epoch'], gamma=params_STAS['lr_decay'])
## Transform
utilPath = osp.join(
    args.basePath,
    args.dataset_name,
...
for epoch in range(args.preEpoch):
    ...
    for batch_idx, (data, seqPrep, _, seqTem, seqPress, seqWind, seqDew) in enumerate(OrdDataLoader):
        ...
        data_dew, dewTarget, dewProbs = updateTSMeBatch4Label(data, seqDew[:,-1], dewRange, args.dropRate, 'dew')
        ...
        for ci, curLength in tqdm.tqdm(
            ...
            ## Crop current scale
            curLength_dew = cropOneLength4EC(data_dew, curLength)
            ## Training
            middleFCN.train()
            C3AE_3D.train()
            ...
            # N1*C1*D*H_u*W_u
            midF = middleFCN(curLength_dew, True)
            probPreds, valuePreds = C3AE_3D(midF)
            
            # BP
            BP_ME_TS(valuePreds, probPreds, dewTarget, dewProbs, dew_optimizer, 'dew')
    ...
    if epoch%params_STAS['training_inv']==0:
        ...
        for batch_idx, (data, seqPrep, _, seqTem, seqPress, seqWind, seqDew) in enumerate(modalGD_FDataLoader):
            ...
            for ci, curLength in tqdm.tqdm(
                ...
                ## Inference
                with torch.no_grad():
                    midF = middleFCN(curLength_dew, False)
                    probPreds, valuePreds = C3AE_3D(midF) 
                    #
                    mse = F.mse_loss(valuePreds, dewTarget).item()
                    kl = klLoss(probPreds, dewProbs).item()                    
                    #
                    d_loss = 0.5 * mse + 0.5 * kl 
                    #
        ...
        if epoch>=3 and d_bestLoss > d_loss_avg:
            ...
            dew_weights = C3AE_3D.state_dict()
            print('saving-epoch-{}-Test-dewLoss-{}'.format(epoch + 1, d_bestLoss))
            # spatial module for temperature and override
            torch.save(dew_weights, osp.join(MTM_save_path, 'dew', 'pretrained-MTM-dew.pth'))
