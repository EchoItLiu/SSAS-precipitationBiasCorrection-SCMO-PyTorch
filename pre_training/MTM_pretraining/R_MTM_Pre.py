...
'''
python pre_training/MTM_pretraining/R_MTM_Pre.py -c ./config/SHO.yaml -r 0.3 -d 8 9
'''
...
rainRange = np.arange(0,xx+xxx, xx)
...
## 5个预训练模型需要的子模块
middleFCN, C3AE_3D = initMTM('rain')
# nnParallel
...
'''
rain_optimizer = torch.optim.Adam([
...
##
scheduler_rain = StepLR(rain_optimizer, step_size=params_STAS['lr_decay_epoch'], gamma=params_STAS['lr_decay'])
## Transform
utilPath = osp.join(
    args.basePath,
    args.dataset_name,
...
for epoch in range(args.preEpoch):
    ...
    for batch_idx, (data, seqPrep, _, seqTem, seqPress, seqWind, seqDew) in enumerate(OrdDataLoader):
        ...
        data_rain, rainTarget, rainProbs = updateTSMeBatch4Label(data, seqPrep[:,-1], rainRange, args.dropRate, 'rain')
        ...
        for ci, curLength in tqdm.tqdm(
            ...
            ## Crop current scale
            curLength_rain = cropOneLength4EC(data_rain, curLength)
            ## Training
            middleFCN.train()
            C3AE_3D.train()
            ...
            # N1*C1*D*H_u*W_u
            midF = middleFCN(curLength_rain, True)
            probPreds, valuePreds = C3AE_3D(midF)
            
            # BP
            BP_ME_TS(valuePreds, probPreds, rainTarget, rainProbs, rain_optimizer, 'rain')
    ...
    if epoch%params_STAS['training_inv']==0:
        ...
        for batch_idx, (data, seqPrep, _, seqTem, seqPress, seqWind, seqDew) in enumerate(modalGD_FDataLoader):
            ...
            ##  N * C * D * H * W  → update  N1 * D * C * H * W  N1*range / N1*range*2  N1*2 N1 / N1*2
            data_rain, rainTarget, rainProbs = updateTSMeBatch4Label(data, seqPrep[:,-1], rainRange, args.dropRate, 'rain')
            rainTarget = rainTarget.unsqueeze(-1)
...
#             print ('ori_data_tem_size:', data_tem.size())
            ...
            for ci, curLength in tqdm.tqdm(
                ...
                ## Inference
                with torch.no_grad():
                    midF = middleFCN(curLength_rain, False)
                    probPreds, valuePreds = C3AE_3D(midF) 
                    #
                    mse = F.mse_loss(valuePreds, rainTarget).item()
                    kl = klLoss(probPreds, rainProbs).item()                    
                    #
                    r_loss = 0.5 * mse + 0.5 * kl 
                    #
...
#             t_bestLoss = r_loss_avg
#             rain_weights = C3AE_3D.state_dict()
#             print('saving-epoch-{}-Test-rainLoss-{}'.format(epoch + 1, r_bestLoss))
#             # spatial module for temperature and override
#             torch.save(rain_weights, osp.join(MTM_save_path, 'rain', 'pretrained-MTM-rain.pth'))
#             torch.save(C3AE_3D, osp.join(MTM_save_path, 'rain', 'pre_C3AE_model_rain.pth'))

        rain_weights = C3AE_3D.state_dict()
        print('saving-epoch-{}-Test-rainLoss-{}'.format(epoch + 1, r_bestLoss))
        # spatial module for temperature and override
        torch.save(rain_weights, osp.join(MTM_save_path, 'rain', 'fix_pretrained-MTM-rain.pth'))
#         torch.save(C3AE_3D, osp.join(MTM_save_path, 'rain', 'pre_C3AE_model_rain.pth'))
