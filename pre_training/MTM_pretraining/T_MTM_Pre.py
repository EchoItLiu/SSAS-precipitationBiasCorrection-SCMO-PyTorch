...
'''
python pre_training/MTM_pretraining/T_MTM_Pre.py -c ./config/SHO.yaml -r 0.3 -d 8 9
'''
...
temRange = np.arange(params_STAS['set_min_CtemValue'],params_STAS['set_max_CtemValue']+params_STAS['set_Ctem_space'], params_STAS['set_Ctem_space'])
...
'''
tem_optimizer = torch.optim.Adam([
...
##
scheduler_tem = StepLR(tem_optimizer, step_size=params_STAS['lr_decay_epoch'], gamma=params_STAS['lr_decay'])
## Transform
utilPath = osp.join(
    args.basePath,
    args.dataset_name,
...
modalOrd = modalOrdinalDataset(args,
    params_STAS['preset_iter'] * params_STAS['pre_train_batch_size'] * len(args.device_ids),
...
OrdDataLoader = data.DataLoader(
    modalOrd, 
    batch_size=params_STAS['pre_train_batch_size'],
    drop_last=True,
    shuffle=True,
    pin_memory=True,
...
modalGD_F = modalGribDataset(args, 
    params_STAS['preset_iter'] * params_STAS['pre_test_batch_size'] * len(args.device_ids),
    params_STAS['resample_range'],
    params_STAS['startDate'],
    params_STAS['endDate'],
    dataset_path,
...
modalGD_FDataLoader = data.DataLoader(
    modalGD_F, 
    batch_size=params_STAS['pre_test_batch_size'],
    drop_last=True,
    shuffle=False,
    pin_memory=True,
    num_workers=args.num_workers,
...
print ('Pre-Training temperature_MTM...')
for epoch in range(args.preEpoch):
    ...
    for batch_idx, (data, seqPrep, _, seqTem, seqPress, seqWind, seqDew) in enumerate(OrdDataLoader):
        ...
        data_tem, temTarget, temProbs = updateTSMeBatch4Label(data, seqTem[:,-1], temRange, args.dropRate, 'tem')
        ...
        for ci, curLength in tqdm.tqdm(
            ...
            ## Crop current scale
            curLength_tem = cropOneLength4EC(data_tem, curLength)
            ## Training
            middleFCN.train()
            C3AE_3D.train()
            ...
            # N1*C1*D*H_u*W_u
            midF = middleFCN(curLength_tem, True)
            probPreds, valuePreds = C3AE_3D(midF)
                  
            # BP
            BP_ME_TS(valuePreds, probPreds, temTarget, temProbs, tem_optimizer, 'tem')
    ...
    if epoch%params_STAS['training_inv']==0:
        ...
        for batch_idx, (data, seqPrep, _, seqTem, seqPress, seqWind, seqDew) in enumerate(modalGD_FDataLoader):
            ...
            ##  N * C * D * H * W  → update  N1 * D * C * H * W  N1*range / N1*range*2  N1*2 N1 / N1*2
            data_tem, temTarget, temProbs = updateTSMeBatch4Label(data, seqTem[:,-1], temRange, args.dropRate, 'tem')
            temTarget = temTarget.unsqueeze(-1)
...
#             print ('ori_data_tem_size:', data_tem.size())
            ...
            for ci, curLength in tqdm.tqdm(
                ...
                ## Inference
                with torch.no_grad():
                    midF = middleFCN(curLength_tem, False)
                    probPreds, valuePreds = C3AE_3D(midF) 
                    #
                    mse = F.mse_loss(valuePreds, temTarget).item()
                    kl = klLoss(probPreds, temProbs).item()                    
                    #
                    t_loss = 0.5 * mse + 0.5 * kl 
                    #
                    t_losses.append(t_loss)
        # 计算当前epoch中的所有minibatch平均loss的均值
        t_loss_avg = np.mean(t_losses)   
        print('epoch-{}-Test-temLoss-{}'.format(epoch + 1, t_loss_avg))
        ## save 5 ME modules  copy.deepcopy(model.state_dict())
        if epoch>=3 and t_bestLoss > t_loss_avg:
            t_bestLoss = t_loss_avg
            tem_weights = C3AE_3D.state_dict()
            print('saving-epoch-{}-Test-temLoss-{}'.format(epoch + 1, t_bestLoss))
            # spatial module for temperature and override
            torch.save(tem_weights, osp.join(MTM_save_path, 'tem', 'pretrained-MTM-tem.pth'))
