...
TS_path = "/mnt/pami14/yqliu/EC-BiasCorrecting/ERA/19_modalsEC"
...
#                         print ('--2--')
            ...
            print ('curr_count_normal-{}:'.format(da_idx), count_normal)
            print ('curr_count_prep-{}:'.format(da_idx), count_prep)
...
#         self.tsLen = ts_length
    ...
    def sliceWindows4LatLon(self):
        ...
        # count
        count_latBound_minmax = []
        count_lonBound_minmax = []
        print ('LATLON_STATIONS_NUM_before:', len(self.latlons_item))
        #
        for k in tqdm.tqdm(
...
#                         print ('slice_ec_shape:', test_slice_ec.shape)
                        if test_slice_ec.shape[3]!=29 or test_slice_ec.shape[2]!=29:
#                             print ('*lat_station:', self.latlons_item[k][1])
#                             print ('*lon_station:', self.latlons_item[k][2])
                            
#                             print ('*i:', i)
#                             print ('*j:', j)
#                             print ('*lat:', lat)
#                             print ('*lon:', lon)
#                             print ('*slice_lon_shape:', test_slice_ec.shape[3])
#                             print ('*slice_lat_shape:', test_slice_ec.shape[2])
                            
                            if test_slice_ec.shape[3]!=29:
                                count_lonBound_minmax.append(lon)
                                
                            if test_slice_ec.shape[2]!=29:
                                count_latBound_minmax.append(lat)
...
#         print ('count_lonBound:', len(count_lonBound_minmax))
#         print ('count_latBound:', len(count_latBound_minmax))
        
        # list → numpy n * N * C * h * w (float32)
        print ('LATLON_STATIONS_NUM_after:', len(self.latlons_item))
    ...
    def gainGT4ecPath(self):  
        ...
        month_path = "{}-19mods-seq".format(seriesNo) + self.monthTimeStamp[:-2]
        ...
        for lth, fileDate in tqdm.tqdm(
                enumerate(self.ec_FeatureDates), total=len(self.ec_FeatureDates),
...
#             modals_filenames = []
            ...
            oldMdhStr = fileDate[2:]
#             oldYearStr = '20'+fileDate[0:2]
            oldYearStr = fileDate[0:2]
...
#             labelDirName = os.path.join(self.micapsPath, '2017', monthStr, "surface", "r6-p")            
           ...
           # test
            ...
            pkl_name = os.path.join(pklFilePath, fileDate+"modals_ec"+".pkl")
