...
TS_path = "/mnt/pami14/yqliu/biasCorrection/18_dataset_lPrep"
...
def PreECF(preTimeStamp):
    ...
    if isinstance(start_time, str):
        ...
        month_days_l = os.listdir(start_time_path)[0:6]
        month_days_l.sort()
...
#         self.stride = round(discard_rainless_ratio / (estimate_average_rainless_num * discard_rainless_ratio), 5)        
    ...
    def gainGT4ecPath(self):
        ...
        month_path = "{}-18e".format(seriesNo) + self.monthTimeStamp[:-2]
...
#             oldYearStr = oldYearStr.replace('-', '')
            ...
            for v, up_index in enumerate(self.update1_indice):   
                ...
                if up_index in indice:
                    ...
                    print ('EC_FEATURE:', ec_features_lth[v].shape)
            ...
            pkl_name = os.path.join(pklFilePath, fileName.split("/")[-3] + fileName.split("/")[-1][-9:-3] + "_ec" + ".pkl")
