...
TS_path = "/mnt/pami23/yqliu/cropdata_modal"
...
#         print ('ec_class_cost_time:', after_time_1 - before_time_1)
        ...
        nf_class_2 = nf_class_2 + nf_class_init 
        nf_class_3_1 = nf_class_3 + nf_class_1 
        nf_class_4_2 = nf_class_4  + nf_class_2
        # pdb.set_trace()
        # a,b,c = len(nf_class_2),len(nf_class_2[0]),len(nf_class_2[0][0])
        # Class_padding = [[[0 for _ in range(c)] for _ in range(b)] for  _ in range(57-3-a)]
        # nf_class_2 = nf_class_2   + Class_padding
        # nf_class_3_1 = nf_class_3 + Class_padding
        # nf_class_4_2 = nf_class_4 + Class_padding
        
#         print ('nf_class_2:', nf_class_2)
        # print ('nf_class_3_1:', nf_class_3_1)
        # print ('nf_class_4_2:', nf_class_4_2)

#         print('before nf_class',len(nf_class_1),len(nf_class_2),len(nf_class_3),len(nf_class_4))
    ...
    def ec_Data(self, nc_interval, before_pc, after_pc, flag_0):
        ...
        ec_featuress = np.stack(ec_features)
        print("EC_DATA_before,size:",ec_featuress.shape)
        ...
        for i in range(35,55):
            ec_features[i]=0
        print("EC_DATA,size:",ec_features.shape)
        ...
        for lth, fileNames in tqdm.tqdm(
            ...
            for v, up_index in enumerate(self.update1_indice): 
                ...
                if up_index in indice_inter:
                    ...
                    # from top to bottom , from left to right
                    lat_range = list(np.arange(lat_lower_bound, lat_upper_bound + 0.125, 0.125)[:17])
                    lat_range = np.array([round(i,2) for i in lat_range]).astype(np.float32)
                    lon_range = list(np.arange(lon_left_bound, lon_right_bound + 0.125, 0.125)[:17])
...
#                     print ('lat_range*:', lat_range.shape)
#                     print ('lon_range*:', lon_range.shape)
